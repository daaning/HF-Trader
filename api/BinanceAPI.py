...
currency = Settings.tradecurrency
...
def get_data(time, currency, rep, loopdone):
    ...
    try:
        if time == 0:
            dataArray = client.get_klines(
                symbol=markets[currency], interval=Client.KLINE_INTERVAL_1HOUR)
        elif time == 1:
            dataArray = client.get_klines(
                symbol=markets[currency], interval=Client.KLINE_INTERVAL_30MINUTE)
        elif time == 2:
            dataArray = client.get_klines(
                symbol=markets[currency], interval=Client.KLINE_INTERVAL_5MINUTE)
        elif time == 3:
            dataArray = client.get_klines(
                symbol=markets[currency], interval=Client.KLINE_INTERVAL_1MINUTE)
